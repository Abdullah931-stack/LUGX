/**
 * Client-only content sanitization (XSS defense in depth).
 *
 * Used inside the browser editor: DOMPurify backed by the native window —
 * zero jsdom bytes in the client bundle. Server-side callers (server
 * actions, import pipeline) use the jsdom-backed ./sanitize module.
 *
 * Allowed tags are the TipTap-default set plus extras needed by imported
 * content. Dangerous elements (script, iframe, style, object, embed,
 * form, input, meta, link, svg/on* handlers) are stripped by DOMPurify.
 */
