...
## 6. Cryptographic Inbound/Outbound Gateway & Quarantine Lifecycle

The sync pipeline integrates `SyncCryptoGateway` (`src/lib/sync/sync-crypto-gateway.ts`) to maintain a strict cryptographic boundary between the local editing surface and network/storage representations:

```mermaid
flowchart LR
    subgraph LocalSurface["Local Plaintext Surface"]
        CM["CodeMirror / EditorAdapter"]
        Diff3["ConflictDialog (Diff3)"]
    end

    subgraph CryptoBoundary["Cryptographic Gateway (SyncCryptoGateway)"]
        Dec["decryptInbound()<br/>(Uses Server IV + MasterKey)"]
        Enc["encryptOutbound()<br/>(Fresh 12-byte CSPRNG IV)"]
    end

    subgraph PersistencePipeline["In-Flight & At-Rest Envelopes"]
        API["Cloud REST API (/api/files/:id)"]
        IDB["Encrypted IndexedDB Storage"]
    end

    API -->|Inbound Ciphertext + Server IV| Dec
    Dec -->|Clean Markdown Plaintext| CM
    Dec -->|Clean Markdown Plaintext| Diff3

    CM -->|Dirty Plaintext| Enc
    Diff3 -->|Resolved Plaintext| Enc
    Enc -->|Ciphertext Base64 + Fresh IV| API
    Enc -->|Ciphertext Base64 + Fresh IV| IDB
```

### 6.1 Inbound Ingestion Pipeline
- **Server IV Extraction**: Inbound remote update broadcasts and HTTP 412 conflict payloads contain ciphertexts generated by remote writers. The gateway extracts `serverVersion.encryptionMetadata.iv` to decrypt the envelope into pure Markdown before passing to UI or merge algorithms.
- **Vault-Lock Quarantine (`CONFLICT_LOCKED`)**: If the user's vault is locked (`!sessionKeyStore.getMasterKeyRaw()`), inbound encrypted payloads are quarantined into `pendingEncryptedConflicts` under `CONFLICT_LOCKED`. Locked encrypted files are filtered out of `pushDirtyFiles`, enabling non-blocking background synchronization of other unencrypted documents. In Phase 23, memory backpressure is bounded by `MAX_QUARANTINED_CONFLICTS = 100` (FIFO oldest eviction via $O(N)$ linear scan, false-eviction immunity), real-time diagnostics (`QuarantineDiagnostics`), and concurrency-safe discard (`discardPendingEncryptedConflict`).
